  ...
  const getHistoryData = async(items) => {
    setEventData(items);
     console.log(items, 'sssssssssssssssssss');
      ...
      if(graphHistoryData)
          Chart.render({ nodes: graphHistoryData.nodes, labels: graphHistoryData.labels, links: graphHistoryData.links }, { ignoreAutoSave: false });
      Chart.loading(false);
      ...
      >
       ...
       <div id="rvinfo">
          <span className="rvchange">
            ...
            <span className="date"> {eventData?.eventFullDate}</span> 
            <span className="info"> {eventData?.eventText}</span>             
          </span>          
              ...
              {/* <Swiper {...params} ref={swiperRef}> */}
                {Object.keys(graphHistory).map((division,key) => (
                   <li  title={`${division}`} key ={key}>
                    { 
                        graphHistory[division].map(function(item, key) {
                         
                            return (<b className ="circle swiper-slide" key={key} onClick={()=> getHistoryData(item)}></b>);
                        })
                    }
