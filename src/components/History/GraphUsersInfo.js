  ...
  const createdUser = graph.users.find((u) => +u.id === +(node.createdUser || graph.userId)) || {
   ...
   return (isEmpty(graph) ? null
    : (
     ...
     <div className="row">
      <span className="item node-name "> {node.name} </span> 
      </div>
      <div className="row"> 
      <span className="item "> <h4>Total change actions: {nodeHistory.length}</h4> </span> 
      ...
      <div className="row info">
        ...
        <span className="item time"> {node.createdAt ? moment(node.createdAt * 1000).format('DD/MM/YYYY hh:mm A') : ''} </span>      
        <div className=""></div>
      </div>
