        ...
        <div className="contributors-container">
            <article key={user.id} className="tooltiptext">
               <Link to={`/profile/${user.id}`} target="_blank" >
                ...
                <div className="info">
                    <div class="username"><Link to={`/profile/${user.id}`} target="_blank" >{`${user.firstName} ${user.lastName}`}</Link></div>
                    <div class="role">Shared  -  { !!(objectId) ? (<a className="tooltipLabel" onClick={() => findLabelInDom(objectId)}>{name}</a>) : ( type )}</div>
