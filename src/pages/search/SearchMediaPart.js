...
class SearchMediaPart extends Component {
  static propTypes = {
    documentSearch: PropTypes.array.isRequired,
    currentUserId: PropTypes.any.isRequired,
    data: PropTypes.array.isRequired,
    history: PropTypes.string.isRequired,
    mediaMode: PropTypes.string.isRequired,
  }
  ...
  componentDidMount() {
    ...
    render() {
      ...
      if (data) {
        data.forEach((d) => {
          [d.node] = d.graphs.nodes.filter((n) => n.id === d.nodeId);
                        ...
                        : (mediaMode === 'document'
                          ? (
                            <a download={document.name} href={document.data}>
                              <p>
                                {' '}
                                {`Ext: ${document.data.substring(document.data.lastIndexOf('.') + 1,
                                  document.data.length)}`}
                                {' '}
                              </p>
                              <p>
                                {' '}
                                {`Desctiption: ${document.description}`}
                              </p>
                            </a>
                          )
                          : (
